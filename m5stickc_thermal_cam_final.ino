...
int WIFI_ENABLE = 0;      // set this variable to 1 to enable WiFi server
int TEMP_THRESHOLD = 40;  // temperature threshold value (NOTE: you may need to adjust this)
...
const char* ssid = "Sarancik-2G";
const char* password = "fishing27";

#define AIO_NAME    "J_Henry"
#define AIO_KEY     "aio_lMMR16QTIcewtBV5eAGOh1naednN"

AdafruitIO_WiFi io(AIO_NAME, AIO_KEY, ssid, password);
AdafruitIO_Feed *blobFeed = io.feed("blobfeed");
...
void setup() {
  ...
  // The byte order can be swapped
  TJpgDec.setSwapBytes(false);

  // The decoder must be given the name of the rendering function above
...
void loop() {
  ...
  /*
  // print pixels array values to the Serial port
  Serial.print("[");
  ...
  */
  ...
  for(int i=0; i<AMG88xx_PIXEL_ARRAY_SIZE; i++){
    ...
    M5.Lcd.fillRect(displayPixelHeight * floor(i / 8), displayPixelWidth * (i % 8),
  ...
  //Serial.print("numBlobsTotal: ");
  //Serial.println(numBlobsTotal);

  // turn on built-in LED if a blob (~person) is detected

   // width and height in pixels of the jpeg:
  uint16_t w = 0, h = 0;
  TJpgDec.getJpgSize(&w, &h, ucDataBlock, sizeof(ucDataBlock));
  TJpgDec.getJpgSize(&w, &h, the_end, sizeof(the_end));
  //if(numBlobs > 0) {
  if(avgVal > 30){
    digitalWrite(M5_LED, LOW);  // turn on LED
    GameAudio.PlayWav(&AudioData, false, 1.0); // play audio
    while(GameAudio.IsPlaying()){
      
       TJpgDec.drawJpg(0, 0, medical_bed, sizeof(medical_bed)); // draw bed
       tft.pushSprite(0, 0);
       delay(500);
  
       TJpgDec.drawJpg(0, 0, the_end, sizeof(the_end)); // draw the end
       tft.pushSprite(0, 0);
       delay(500);
    }
  }
  ...
  M5.Lcd.fillRect(0, 80, 80, 160, BLACK);
